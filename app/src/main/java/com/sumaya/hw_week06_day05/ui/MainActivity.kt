    ...
    private fun loadMoviesData(query: String? = null) {
        vm.fetchInterestingList(query).observe(this, {
            if(query.isNullOrEmpty()){
                moviesRV.adapter = MovieRVAdapter(it.results)
            } else {
                moviesRV.swapAdapter(MovieRVAdapter(it.results), false)
            }
            Log.d("Tha Movie Main Response", it.results.toString())
        })
    }

//    I tried to delete but did not work :(

//    private fun deleteMovieList(query: String? = null){
//        vm.deleteMovieList(query).observe(this, {
//            if(query.isNullOrEmpty()){
//                moviesRV.adapter = MovieRVAdapter(it.results)
//            } else {
//                moviesRV.removeOnItemTouchListener(moviesRV)
//            }
//            Log.d("Tha Movie Main Response", it.results.toString())
//        })
//    }
